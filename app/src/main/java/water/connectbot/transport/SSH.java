...
package water.connectbot.transport;
...
import water.connectbot.bean.PortForwardBean;
import water.connectbot.bean.PubkeyBean;
import water.connectbot.service.TerminalBridge;
import water.connectbot.service.TerminalManager;
import water.connectbot.util.Ed25519Provider;
import water.connectbot.util.HostDatabase;
import water.connectbot.util.PubkeyDatabase;
import water.connectbot.util.PubkeyUtils;
...
public class SSH extends AbsTransport implements ConnectionMonitor, InteractiveCallback, AuthAgentCallback {
	...
	public class HostKeyVerifier extends ExtendedServerHostKeyVerifier {
		public boolean verifyServerHostKey(String hostname, int port,
			...
			case KnownHosts.HOSTKEY_IS_OK:
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_sucess, algorithmName, fingerprint));
				return true;

			case KnownHosts.HOSTKEY_IS_NEW:
				// prompt user
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.host_authenticity_warning, hostname));
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.host_fingerprint, algorithmName, fingerprint));

				result = bridge.promptHelper.requestBooleanPrompt(null, manager.res.getString(water.connectbot.R.string.prompt_continue_connecting));
				if (result == null) {
					return false;
				}
				if (result) {
			...
			case KnownHosts.HOSTKEY_HAS_CHANGED:
				String header = String.format("@   %s   @",
						manager.res.getString(water.connectbot.R.string.host_verification_failure_warning_header));
				...
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.host_verification_failure_warning));

				bridge.outputLine(String.format(manager.res.getString(water.connectbot.R.string.host_fingerprint),
			...
			default:
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_failed));
				return false;
			}
		}
	...
	private void authenticate() {
		...
		bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_auth));
			...
			if (!pubkeysExhausted &&
				...
				if (pubkeyId == HostDatabase.PUBKEYID_ANY) {
					...
					bridge.outputLine(manager.res
							.getString(water.connectbot.R.string.terminal_auth_pubkey_any));
					...
					bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_auth_pubkey_specific));
					// use a specific key for this host, as requested
					PubkeyBean pubkey = manager.pubkeydb.findPubkeyById(pubkeyId);

					if (pubkey == null)
						bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_auth_pubkey_invalid));
				...
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_auth_ki));
				interactiveCanContinue = false;
				if (connection.authenticateWithKeyboardInteractive(host.getUsername(), this)) {
					finishConnection();
				} else {
					bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_auth_ki_fail));
				}
			} else if (connection.isAuthMethodAvailable(host.getUsername(), AUTH_PASSWORD)) {
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_auth_pass));
				String password = null;

				if (!TextUtils.isEmpty(host.getPassword())) {
					//password is save
					Boolean result = bridge.getPromptHelper().requestBooleanPrompt(null,
							manager.res.getString(water.connectbot.R.string.prompt_use_saved_password));
					if (result == null || result) {
						//conform
						...
						password = bridge.getPromptHelper().requestStringPrompt(null,
								manager.res.getString(water.connectbot.R.string.prompt_password));
					}
				} else {
					//input password
					password = bridge.getPromptHelper().requestStringPrompt(null,
				...
				if (password != null
					...
					bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_auth_pass_fail));
				}
			} else {
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_auth_fail));
			}
	...
	private boolean tryPublicKey(PubkeyBean pubkey) throws NoSuchAlgorithmException, InvalidKeySpecException, IOException {
		...
		if (manager.isKeyLoaded(pubkey.getNickname())) {
			...
			if (pubkey.isEncrypted()) {
				password = bridge.getPromptHelper().requestStringPrompt(null,
						manager.res.getString(water.connectbot.R.string.prompt_pubkey_password, pubkey.getNickname()));
	...
	private boolean tryPublicKey(String username, String keyNickname, KeyPair pair) throws IOException {
		...
		if (!success)
			bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_auth_pubkey_fail, keyNickname));
		return success;
	}
	...
	private void finishConnection() {
		...
		for (PortForwardBean portForward : portForwards) {
			try {
				...
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_enable_portfoward, portForward.getDescription()));
			} catch (Exception e) {
		...
		if (!host.getWantSession()) {
			bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_no_session));
			bridge.onConnected();
			return;
		}
	...
	public void connect() {
		...
		try {
			...
			*/
			...
			bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_kex_algorithm,
			...
			if (connectionInfo.clientToServerCryptoAlgorithm
				...
				bridge.outputLine(manager.res.getString(water.connectbot.R.string.terminal_using_algorithm,
						connectionInfo.clientToServerCryptoAlgorithm,
						connectionInfo.clientToServerMACAlgorithm));
			} else {
				bridge.outputLine(manager.res.getString(
						water.connectbot.R.string.terminal_using_c2s_algorithm,
						connectionInfo.clientToServerCryptoAlgorithm,
						connectionInfo.clientToServerMACAlgorithm));

				bridge.outputLine(manager.res.getString(
						water.connectbot.R.string.terminal_using_s2c_algorithm,
	...
	public static String getFormatHint(Context context) {
		return String.format("%s@%s:%s",
				context.getString(water.connectbot.R.string.format_username),
				context.getString(water.connectbot.R.string.format_hostname),
				context.getString(water.connectbot.R.string.format_port));
	}
	...
	private boolean promptForPubkeyUse(String nickname) {
		Boolean result = bridge.promptHelper.requestBooleanPrompt(null,
				manager.res.getString(water.connectbot.R.string.prompt_allow_agent_to_use_key,
						nickname));
		return result;
	}
