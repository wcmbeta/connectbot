...
package water.connectbot.views;
...
public class CheckableMenuItem extends RelativeLayout {
	...
	public CheckableMenuItem(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mRootView = inflate(context, water.connectbot.R.layout.view_checkablemenuitem, this);

		mTitle = (TextView) mRootView.findViewById(water.connectbot.R.id.title);
		mSummary = (TextView) mRootView.findViewById(water.connectbot.R.id.summary);
		mSwitch = (SwitchCompat) findViewById(water.connectbot.R.id.checkbox_switch);
		...
		if (attrs != null) {
			TypedArray typedArray = getContext().obtainStyledAttributes(
					attrs, water.connectbot.R.styleable.CheckableMenuItem, 0, 0);

			@DrawableRes int iconRes = typedArray.getResourceId(
					water.connectbot.R.styleable.CheckableMenuItem_android_icon, 0);
			@StringRes int titleRes = typedArray.getResourceId(
					water.connectbot.R.styleable.CheckableMenuItem_android_title, 0);
			@StringRes int summaryRes = typedArray.getResourceId(
					water.connectbot.R.styleable.CheckableMenuItem_summary, 0);

			typedArray.recycle();

			ImageView icon = (ImageView) mRootView.findViewById(water.connectbot.R.id.icon);
