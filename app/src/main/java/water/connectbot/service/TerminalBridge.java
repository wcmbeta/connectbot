...
package water.connectbot.service;
...
import water.connectbot.TerminalView;
import water.connectbot.bean.HostBean;
import water.connectbot.bean.PortForwardBean;
import water.connectbot.bean.SelectionArea;
import water.connectbot.transport.AbsTransport;
import water.connectbot.transport.TransportFactory;
...
public class TerminalBridge implements VDUDisplay {
	...
	protected void startConnection() {
		...
		outputLine(manager.res.getString(water.connectbot.R.string.terminal_connecting, host.getHostname(), host.getPort(), host.getProtocol()));
	...
	public void dispatchDisconnect(boolean immediate) {
		...
		if (immediate || (host.getQuickDisconnect() && !host.getStayConnected())) {
			...
			{
				final String line = manager.res.getString(water.connectbot.R.string.alert_disconnect_msg);
			...
			Thread disconnectPromptThread = new Thread(new Runnable() {
				public void run() {
					Boolean result = promptHelper.requestBooleanPrompt(null,
							manager.res.getString(water.connectbot.R.string.prompt_host_disconnected));
